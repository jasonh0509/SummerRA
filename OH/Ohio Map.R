...
data=read.csv("https://raw.githubusercontent.com/jasonh0509/SummerRA/main/OHDATAnospace.csv",header = TRUE)
data<-subset(data,select = c(CountyName,Year,Code,Population,Deaths))
data$death.rate = data$Deaths/data$Population*100
data$FIPS = data$Code-39000
...
ui <- fluidPage(
  titlePanel(p("Opioid Related Death Rate (Ohio)", style = "color:#3474A7")),
  sidebarLayout(
    sidebarPanel(
      selectInput(
